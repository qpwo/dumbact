...
function serialize(node) {
  ...
  const attrs = Object.keys(node.attributes || {}).map(k => ` ${k}="${escapeHtml(node.attributes[k])}"`).join("");
...
function tick() { return Promise.resolve().then(() => Dumbact.flush()); }
function host() { return new FakeDocument().createElement("main"); }
...
(async function main() {
  ...
  await test("sub immediate and unsubscribe", () => {
    ...
    unsub();
    Dumbact.set("x", 3);
    assert.deepEqual(seen, [[1, "x", undefined], [2, "x", 1]]);
  });
  ...
  await test("function components receive children", () => {
    const root = host();
    function Box(props) { return h("section", null, props.children); }
  ...
  await test("jsx runtime reads props.children", () => {
    const root = host();
    const node = Dumbact.jsx("div", { children: [Dumbact.jsx("span", { children: "x" })] });
  ...
  await test("auto-tracked id reads rerender without hooks", async () => {
    ...
    let renders = 0;
    Dumbact.set("count", 1);
    Dumbact.render(() => { renders++; return h("button", null, "count=", Dumbact.get("count")); }, root);
  ...
  await test("peek does not subscribe current render", async () => {
    ...
    Dumbact.render(() => { renders++; return h("p", null, Dumbact.peek("silent")); }, root);
  ...
  await test("duplicate set neither notifies nor rerenders", async () => {
    ...
    let notices = 0;
    Dumbact.set("same", 1);
    const unsub = Dumbact.sub("same", () => { notices++; });
    Dumbact.render(() => { renders++; return h("p", null, Dumbact.get("same")); }, root);
    Dumbact.set("same", 1);
    await tick();
    assert.equal(renders, 1);
  ...
  await test("event handlers use native addEventListener and update state", async () => {
    ...
    Dumbact.render(() => h("button", { onClick: () => Dumbact.set("clicks", n => n + 1) }, "clicks ", Dumbact.get("clicks")), root);
  ...
  await test("event handler replacement and removal", () => {
    const root = host();
    let n = 0;
    Dumbact.render(h("button", { onClick: () => { n += 1; } }, "x"), root);
    const button = elementChildren(root)[0];
    button.dispatchEvent({ type: "click" });
    Dumbact.render(h("button", { onClick: () => { n += 10; } }, "x"), root);
    button.dispatchEvent({ type: "click" });
  ...
  await test("child reads shared id directly, no prop drilling", async () => {
    const root = host();
    function DeepChild() { return h("strong", null, Dumbact.get("user:name", "?")); }
    function Parent() { return h("div", null, h("aside", null, h(DeepChild))); }
  ...
  await test("keyed children reorder by moving existing DOM nodes", () => {
    const root = host();
    function list(items) {
      return h("ul", null, items.map(item => h("li", { key: item, "data-k": item }, item)));
    }
    ...
    const order = ul.childNodes.filter(n => n.nodeType === 1).map(n => n.textContent).join("");
  ...
  await test("fragments render multiple siblings without element wrapper", () => {
    ...
    assert.equal(text(root), "AB");
    assert.deepEqual(elementChildren(root).map(n => n.localName), ["i", "b"]);
  });

  await test("fragment children can reorder keyed ranges", () => {
    const root = host();
    function Item(props) { return h(Fragment, { key: props.id }, h("dt", null, props.id), h("dd", null, props.value)); }
    function view(order) { return h("dl", null, order.map(x => h(Item, { key: x, id: x, value: x.toUpperCase() }))); }
  ...
  await test("attributes, className, boolean, value, checked", () => {
    const root = host();
    Dumbact.render(h("input", { className: "x", disabled: true, value: "a", checked: true, "data-g": "1" }), root);
  ...
  await test("raw HTML props are ignored", () => {
    const root = host();
    Dumbact.render(h("div", { html: "<span>raw</span>", dangerouslySetInnerHTML: { __html: "<i>raw</i>" }, unsafeHTML: "<b>raw</b>" }, h("em", null, "safe")), root);
    const div = elementChildren(root)[0];
  ...
  await test("unmount removes cell view membership", async () => {
    ...
    let renders = 0;
    Dumbact.set("live", 1);
    Dumbact.render(() => { renders++; return h("p", null, Dumbact.get("live")); }, root);
    Dumbact.unmount(root);
    Dumbact.set("live", 2);
    await tick();
    assert.equal(renders, 1);
  ...
  await test("render errors are captured in sys:errors", () => {
    const root = host();
    assert.throws(() => Dumbact.render(() => { throw new Error("bad render goblin"); }, root), /bad render goblin/);
