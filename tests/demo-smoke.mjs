...
async function start(rel) {
  ...
  return {
    ...
    stop: () => new Promise(done => {
      ...
      child.kill('SIGTERM');
      setTimeout(() => { if (child.exitCode === null && child.signalCode === null) child.kill('SIGKILL'); finish(); }, 1000).unref();
    })
  };
}
...
class FakeNode {
  constructor(doc) { this.ownerDocument = doc || null; this.parentNode = null; this.childNodes = []; }
  get firstChild() { return this.childNodes[0] || null; }
  get nextSibling() { if (!this.parentNode) return null; const a = this.parentNode.childNodes; const i = a.indexOf(this); return i >= 0 ? a[i + 1] || null : null; }
  ...
  removeChild(node) { const i = this.childNodes.indexOf(node); if (i < 0) throw new Error('removeChild target is not a child'); this.childNodes.splice(i, 1); node.parentNode = null; return node; }
  get textContent() { return this.childNodes.map(n => n.textContent).join(''); }
  set textContent(value) { while (this.firstChild) this.removeChild(this.firstChild); if (value !== '') this.appendChild(this.ownerDocument.createTextNode(String(value))); }
}
class FakeText extends FakeNode { constructor(doc, value) { super(doc); this.nodeType = 3; this.nodeName = '#text'; this.nodeValue = String(value); } get textContent() { return this.nodeValue; } set textContent(value) { this.nodeValue = String(value); } }
class FakeComment extends FakeNode { constructor(doc, value) { super(doc); this.nodeType = 8; this.nodeName = '#comment'; this.nodeValue = String(value); } get textContent() { return ''; } set textContent(_) {} }
class FakeElement extends FakeNode {
  constructor(doc, tag) {
    super(doc);
    this.nodeType = 1; this.localName = String(tag).toLowerCase(); this.tagName = String(tag).toUpperCase(); this.nodeName = this.tagName;
    this.attributes = Object.create(null); this.style = Object.create(null); this.dataset = Object.create(null);
    this.id = ''; this.value = ''; this.checked = false; this.disabled = false; this.className = ''; this.__listeners = Object.create(null);
  }
  setAttribute(name, value) { value = String(value); this.attributes[name] = value; if (name === 'id') this.id = value; if (name === 'class') this.className = value; if (name.startsWith('data-')) this.dataset[name.slice(5).replace(/-([a-z])/g, (_, c) => c.toUpperCase())] = value; }
  getAttribute(name) { return Object.prototype.hasOwnProperty.call(this.attributes, name) ? this.attributes[name] : null; }
  hasAttribute(name) { return Object.prototype.hasOwnProperty.call(this.attributes, name); }
  removeAttribute(name) { delete this.attributes[name]; if (name === 'id') this.id = ''; if (name === 'class') this.className = ''; }
  setAttributeNS(_ns, name, value) { this.setAttribute(name, value); }
  removeAttributeNS(_ns, name) { this.removeAttribute(name); }
  addEventListener(type, fn) { (this.__listeners[type] || (this.__listeners[type] = new Set())).add(fn); }
  removeEventListener(type, fn) { if (this.__listeners[type]) this.__listeners[type].delete(fn); }
  dispatchEvent(event) { event.target = event.target || this; event.currentTarget = this; for (const fn of Array.from(this.__listeners[event.type] || [])) fn.call(this, event); return !event.defaultPrevented; }
  get innerHTML() { return this.childNodes.map(n => n.textContent).join(''); }
  set innerHTML(_value) { while (this.firstChild) this.removeChild(this.firstChild); }
}
class FakeDocument extends FakeNode {
  constructor() { super(null); this.ownerDocument = this; this.nodeType = 9; this.readyState = 'complete'; this.documentElement = this.createElement('html'); this.body = this.createElement('body'); this.appendChild(this.documentElement); this.documentElement.appendChild(this.body); }
  createElement(tag) { return new FakeElement(this, tag); }
  createElementNS(_ns, tag) { return new FakeElement(this, tag); }
  createTextNode(value) { return new FakeText(this, value); }
  createComment(value) { return new FakeComment(this, value); }
  addEventListener() {}
  querySelector(selector) {
    if (selector[0] === '#') return find(this, n => n.nodeType === 1 && n.id === selector.slice(1));
    if (selector[0] === '[' && selector.endsWith(']')) { const name = selector.slice(1, -1).split('=')[0]; return find(this, n => n.nodeType === 1 && n.hasAttribute(name)); }
    return find(this, n => n.nodeType === 1 && n.localName === selector.toLowerCase());
  }
}
function find(node, pred) { if (pred(node)) return node; for (const child of node.childNodes) { const got = find(child, pred); if (got) return got; } return null; }
function fakeStorage() { const m = new Map(); return { getItem: k => m.has(String(k)) ? m.get(String(k)) : null, setItem: (k, v) => { m.set(String(k), String(v)); }, removeItem: k => { m.delete(String(k)); }, clear: () => m.clear() }; }
function fakeResponse(data, status = 200, headers = {}) { return { ok: status >= 200 && status < 300, status, headers: { get: k => headers[String(k).toLowerCase()] || null }, json: async () => data, text: async () => typeof data === 'string' ? data : JSON.stringify(data) }; }
function fakeFetch(url, opts = {}) {
  url = String(url);
  if (url.includes('/api/notes') && opts.method === 'POST') return Promise.resolve(fakeResponse({ id: 'x', text: 'new note' }, 201));
  ...
  if (url.includes('/api/actions')) return Promise.resolve(fakeResponse([{ id: '1', title: 'Pin goblin dashboard', hint: 'folklore' }]));
  return Promise.resolve(fakeResponse({ ok: true }));
}
class FakeEventSource {
  constructor() { this.listeners = Object.create(null); setTimeout(() => { if (this.onopen) this.onopen({ type: 'open' }); this.emit('metric', { count: 1, cpu: 42, mem: 27 }); }, 0); }
  addEventListener(type, fn) { (this.listeners[type] || (this.listeners[type] = new Set())).add(fn); }
  emit(type, data) { for (const fn of Array.from(this.listeners[type] || [])) fn({ type, data: JSON.stringify(data) }); }
  close() {}
}
async function settle() { for (let i = 0; i < 6; i++) { await new Promise(r => setTimeout(r, 0)); Dumbact.flush(); } }
async function runHtml(name) {
  ...
  const timers = [];
  const previous = { document: globalThis.document, window: globalThis.window, localStorage: globalThis.localStorage, fetch: globalThis.fetch, EventSource: globalThis.EventSource, requestAnimationFrame: globalThis.requestAnimationFrame, cancelAnimationFrame: globalThis.cancelAnimationFrame, performance: globalThis.performance, Dumbact: globalThis.Dumbact };
  Object.assign(globalThis, {
    document: doc,
    window: { document: doc, addEventListener() {}, removeEventListener() {}, matchMedia: () => ({ matches: false, addEventListener() {}, removeEventListener() {} }) },
    localStorage: fakeStorage(),
    fetch: fakeFetch,
    EventSource: FakeEventSource,
    requestAnimationFrame: fn => { const id = setTimeout(() => fn(Date.now()), 0); timers.push(id); return id; },
  ...
  try {
    ...
    await settle();
    assert.equal(doc.documentElement.dataset.demoReady, name.slice(0, 2), name + ' ready flag');
...
function checkHtml(name, html) {
  ...
  for (const script of scripts(html)) {
    const mode = Dumbact.scriptTypes[attr(script.attrs, 'type').toLowerCase().trim()];
    if (mode && !/-module$/.test(mode)) assert.doesNotThrow(() => new Function(Dumbact.compile(script.text, mode)), name + ' compile script');
    if (mode && /-module$/.test(mode)) assert.doesNotThrow(() => Dumbact.compile(script.text, mode), name + ' compile module script');
  }
}
...
for (const [mode, source] of [
  ['js', 'const x = 1;'],
  ['ts', 'type X = { a: string }; const f = (x: number) => x + 1; const ok = true, a = () => 1, b = 2; const y = ok ? a() : b;'],
...
for (const htmlName of htmlNames) {
  ...
  if (moduleSources[htmlName]) {
    for (const [file, mode] of moduleSources[htmlName]) {
      ...
      if (file === 'modules/cdn-main.tsx') assert.match(source, /https:\/\/cdn\.jsdelivr\.net\/npm\/lodash-es@4\.17\.21\/chunk\.js/, 'CDN module URL');
    }
...
await withServer(serverNames[0], async server => {
  const created = await fetch(server.url + 'api/notes', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ text: 'server smoke note' }) });
...
await withServer(serverNames[1], async server => {
  ...
  const first = await Promise.race([reader.read(), new Promise((_, reject) => setTimeout(() => reject(new Error('SSE timeout')), 1500))]);
  assert.ok(Buffer.from(first.value || []).toString('utf8').includes('event: metric'));
