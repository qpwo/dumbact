...
async function cleanupProcesses() {
  ...
  for (const pid of entries) {
    ...
    if (/playwright_chromiumdev_profile|chrome_crashpad_handler|dumbact-module-pack\/demos\/(07|08|09|10)-/.test(cmd)) victims.push(Number(pid));
  }
...
function run(args) {
  return new Promise((resolve, reject) => {
    const child = spawn(args[0], args.slice(1), { cwd: process.cwd(), env: process.env, stdio: ['ignore', 'pipe', 'pipe'] });
