;(function dumbactFactory(global) {
  ...
  function scope(prefix) {
    ...
    function id(key) {
      return prefix + sep + String(key)
    }
    return {
      id: id,
      get: function scopedGet(key, fallback) {
        return get(id(key), fallback)
      },
      peek: function scopedPeek(key, fallback) {
        return peek(id(key), fallback)
      },
      set: function scopedSet(key, patch) {
        return set(id(key), patch)
      },
      del: function scopedDel(key) {
        return del(id(key))
      },
      sub: function scopedSub(key, listener, immediate) {
        return sub(id(key), listener, immediate)
      },
      clear: function scopedClear() {
        clear(prefix + sep)
      }
    }
  }
  ...
  function unmount(host) {
    ...
    root.queued = false
    queue.delete(root)
    root.deps.forEach(function each(c) {
      c.views.delete(root)
      prune(c)
    })
    root.deps.clear()
...
;(function dumbactTinyCompiler(global) {
  ...
  function compile(source, mode) {
    ...
    var ts =
      mode.indexOf('ts') >= 0 || mode.indexOf('typescript') >= 0 || mode === 'text/dumbact' || mode === 'text/notbabel'
  ...
  function removeTypeAliases(src) {
    ...
    for (var i = 0; i < lines.length; i++) {
      if (!isTypeAliasStart(lines[i])) {
        out += lines[i] + (i + 1 < lines.length ? '\n' : '')
        continue
      }
      var chunk = lines[i]
      var depth = typeAliasDepth(chunk)
      while (i + 1 < lines.length && typeAliasContinues(chunk, lines[i + 1], depth)) {
        i++
        chunk += '\n' + lines[i]
  ...
  function isTypeAliasStart(line) {
    var s = line.trimStart()
    if (s.indexOf('export type ') === 0) s = s.slice('export '.length)
    if (s.indexOf('type ') !== 0) return false
    return s.indexOf('=') >= 0
  }

  function typeAliasContinues(chunk, next, depth) {
    if (depth > 0) return true
    var last = lastNonemptyLine(chunk).trimEnd()
    if (!last) return false
    if (last[last.length - 1] === ';') return false
    if (last[last.length - 1] === '=') return true
    if (last[last.length - 1] === '|') return true
    if (last[last.length - 1] === '&') return true
    var s = next.trimStart()
    if (!s) return false
    return s[0] === '|' || s[0] === '&' || s[0] === ';'
  }
  ...
  function typeAliasDepth(line) {
    var depth = 0
    for (var i = 0; i < line.length; i++) {
      var ch = line[i]
      if (ch === '"' || ch === "'" || ch === '`') {
        i = readQuoted(line, i).end
        continue
      }
  ...
  function stripColonTypes(src) {
    ...
    for (var i = 0; i < src.length; i++) {
      ...
      if (ch === '/' && src[i + 1] === '*') {
        ...
        if (ce < 0) ce = src.length - 2
        out += src.slice(i, ce + 2)
        i = ce + 1
        continue
      }
  ...
  function transformJSX(source) {
    ...
    for (var i = 0; i < src.length; ) {
      ...
      if (ch === '<' && canStartJSX(src, i)) {
        var node = parseJSX(src, i)
        out += node.code
        i = node.end
        continue
      }
  ...
  function normalizeText(t) {
    ...
    if (lines.length === 1) return lines[0].replace(/\s+/g, ' ')
    var out = []
    for (var i = 0; i < lines.length; i++) {
      var x = lines[i].replace(/\s+/g, ' ').trim()
      if (x) out.push(x)
    }
    return out.join(' ')
  }
  ...
  function readQuoted(src, i) {
    ...
    for (; j < src.length; j++) {
      if (src[j] === '\\') j++
        ...
        j = findMatching(src, j + 1, '{', '}')
        if (j < 0) j = src.length
      }
    }
    return {
      text: src.slice(i, Math.min(src.length, j + 1)),
      end: Math.min(src.length - 1, j)
    }
  }
  ...
  function prevSig(src, i) {
    for (var j = i - 1; j >= 0; j--) if (!/\s/.test(src[j])) return src[j]
    return ''
  }
  function nextSig(src, i) {
    for (var j = i; j < src.length; j++) if (!/\s/.test(src[j])) return src[j]
    return ''
  }
  ...
  function runScripts(doc) {
    ...
    return list.reduce(function (p, script) {
      return p.then(function (out) {
        return runScript(script).then(function (r) {
          out.push(r)
          return out
        })
      })
  ...
  function runScript(script) {
    ...
    var base = src
      ? new URL(src, (doc && doc.baseURI) || (global.location && global.location.href) || 'http://dumbact.local/').href
      : (doc && doc.baseURI) || (global.location && global.location.href) || 'http://dumbact.local/inline.html'

    if (isModuleMode(mode)) {
      var urlPromise = src
        ? moduleURL(base, mode)
        : moduleSourceURL(script.textContent || '', base, mode, 'dumbact-inline.' + cleanMode(mode))
      return urlPromise.then(function (url) {
        return new Promise(function (resolve, reject) {
          ...
          real.onerror = function () {
            reject(new Error('Dumbact module failed to load: ' + url))
          }
    ...
    var sourcePromise = src
      ? global.fetch(base).then(function (r) {
          if (!r.ok) throw new Error('Dumbact could not fetch ' + base + ': ' + r.status)
          return r.text()
        })
  ...
  function resolveModuleSpecifier(spec, base, mode) {
    ...
    if (/^(?:[a-z][a-z0-9+.-]*:|\/|\.\.?\/)/i.test(spec)) {
      ...
      if (/^https?:|^file:|^blob:|^data:/.test(url) && /\.(tsx|jsx|ts|mjs|js)(?:[?#]|$)/i.test(url))
        return moduleURL(url, inferModeFromUrl(url, mode))
  ...
  function objectURL(code) {
    if (!global.Blob || !global.URL || !global.URL.createObjectURL)
      throw new Error('Dumbact module loader requires Blob and URL.createObjectURL')
  ...
  function moduleSpecifiers(code) {
    ...
    while (i < code.length) {
      ...
      if (ch === '/' && code[i + 1] === '*') {
        ...
        i = ce < 0 ? code.length : ce + 2
        continue
      }
      if (wordAt(code, i, 'from')) {
        i = collectAfterFrom(code, i + 4, out)
        continue
      }
      if (wordAt(code, i, 'import')) {
        i = collectAfterImport(code, i + 6, out)
        continue
      }
      i++
    }
  ...
  function wordAt(src, i, word) {
    return (
      src.slice(i, i + word.length) === word &&
      !/[A-Za-z0-9_$]/.test(src[i - 1] || '') &&
      !/[A-Za-z0-9_$]/.test(src[i + word.length] || '')
    )
  }
  ...
  function scriptMode(script) {
    return (
      scriptTypes[
        String(script.getAttribute('type') || '')
          .toLowerCase()
          .trim()
      ] || ''
    )
  ...
  function boot() {
    runScripts(global.document).catch(function (error) {
      if (D.set)
        D.set('sys:errors', function (old) {
          return (old || []).concat(error)
        })
      setTimeout(function () {
        throw error
      }, 0)
